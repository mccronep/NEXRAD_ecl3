...
###
### MODIFIED BY: Paul McCrone 9/5/2023
###
...
if len(sys.argv) == 1:
        print "\n"
        print """This python 2 only program is a shortcut script to establish a communications (comms) link in an ORPG# account using
the nbtcp command to generate Level-III products, after the MRPG has been successfully configured and
started. Note that the user MUST of course already be logged in to a valid ORPG# account, and the files
$HOME/cfg/comms_link.conf and $HOME/cfg/tcp.conf MUST be already configured for the appropriate port number
prior to starting the MRPG and running this script. Also ensure that a valid RPS listing (.dat file) of
...
try:
    ...
    #from subprocess import PIPE as PIP
    #from subprocess import STDOUT as STOUT
...
p=Popen([cmd_spec, " -p ", port_num, " -A ", " -s ",rad_site, " -d ",output_dir," rpga1 "], stdout=PIPE, stdin=PIPE, stderr=STDOUT)
print("P status at the start")
print(p)
print "Popen returncode part A= "+str(p.returncode)
#nbtcp_stdout=p.communicate(input="/export/home/orpg2/cfg/qpetest_rps_list.ISDP_B22.dat\n3\n1\n")[0]
nbtcp_stdout1=p.communicate(input=b'/export/home/orpg2/cfg/qpetest_rps_list.ISDP_B22.dat\n')[0]
print("P status at the first input")
print(p)
print "Popen returncode part B= "+str(p.returncode)
nbtcp_stdout2=p.communicate(input=b'3\n')[0]
nbtcp_stdout3=p.communicate(input=b'1\n')[0]



print "Popen returncode part C= "+str(p.returncode)
